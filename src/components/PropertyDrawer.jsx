...
function LogoImg({ company }) {
  ...
  return (
    <img
      src={`https://logos.gentz.co/logo/by-name/${encodeURIComponent(company)}`}
      alt={company}
      onError={() => setErr(true)}
...
export default function PropertyDrawer({ property, onClose, updateProperty }) {
  ...
  // Load notes from gentz_notes (shared with Campo/Birdie/Topo)
  useEffect(() => {
    if (!p) return
    const q = query(
      collection(db, 'gentz_notes'),
      where('linkedProperties', 'array-contains', propId)
    )
  ...
  const addNote = async () => {
    ...
    // If it's a call, also log it on the property CRM record
    if (noteType === 'call') {
      const callLog = [...(p.callLog || []), {
  ...
  const deleteNote = async (docId) => {
    await deleteDoc(doc(db, 'gentz_notes', docId))
  }
  ...
  return (
    <div style={{
      width: '340px', background: '#0d1526', borderLeft: '1px solid #1e2d47',
      display: 'flex', flexDirection: 'column', flexShrink: 0, overflowY: 'auto', zIndex: 10,
    }}>
      {/* Header */}
      <div style={{ padding: '16px', borderBottom: '1px solid #1e2d47', background: '#0a1122' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
          ...
          <button onClick={onClose} style={{
            background: '#1e2d47', border: 'none', color: '#64748b', cursor: 'pointer',
            borderRadius: '4px', width: '24px', height: '24px', fontSize: '14px', flexShrink: 0
          }}>×</button>
        </div>

        <select
          value={p.callStatus}
          onChange={e => updateProperty(p.id, { callStatus: e.target.value })}
          style={{
            width: '100%', padding: '6px 10px', borderRadius: '6px', border: `1px solid ${statusOpt.color}`,
            background: `${statusOpt.color}22`, color: statusOpt.color, fontSize: '12px', fontWeight: 600,
        ...
        {p.forSale && (
          <div style={{ marginTop: '8px', padding: '6px 10px', background: 'rgba(248,113,113,0.15)', border: '1px solid rgba(248,113,113,0.3)', borderRadius: '6px', textAlign: 'center' }}>
            <span style={{ color: '#f87171', fontSize: '12px', fontWeight: 700 }}>
              🔴 FOR SALE {p.forSalePrice > 0 ? `— ${fmt$(p.forSalePrice)}` : ''}
            </span>
          </div>
        )}
      </div>
      ...
      <div style={{ padding: '12px 16px', borderBottom: '1px solid #1e2d47' }}>
        ...
        {row('Owner Name', p.owner !== p.trueOwner ? p.owner : null)}
      ...
      {/* Notes — from gentz_notes, shared with Campo/Birdie/Topo */}
      <div style={{ padding: '12px 16px', flex: 1 }}>
        <div style={{ fontSize: '10px', color: '#475569', letterSpacing: '0.1em', marginBottom: '8px' }}>
          NOTES & ACTIVITY
          <span style={{ marginLeft: '6px', color: '#334155', fontSize: '9px' }}>synced with Campo · Birdie · Topo</span>
        </div>
        ...
        <button onClick={addNote} style={{
          width: '100%', padding: '8px', background: '#f59e0b', border: 'none', borderRadius: '5px',
          color: '#000', fontWeight: 700, fontSize: '12px', cursor: 'pointer', marginBottom: '12px'
        }}>
          + SAVE NOTE
        </button>
          ...
          <div key={n._docId} style={{ padding: '8px', background: '#1a2540', borderRadius: '6px', marginBottom: '6px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
              <span style={{ fontSize: '10px', color: '#64748b' }}>
                {typeIco[n.type] || '📝'} {n.type} · {n.timestamp?.slice(0, 10)} · {n.source || 'storvault'}
              </span>
              <button onClick={() => deleteNote(n._docId)}
                style={{ background: 'none', border: 'none', color: '#475569', cursor: 'pointer', fontSize: '11px', padding: 0 }}>✕</button>
            </div>
